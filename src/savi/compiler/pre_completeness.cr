##
# The purpose of the PreCompleteness pass is to prove that constructors initialize
...
#
module Savi::Compiler::PreCompleteness
  struct Analysis
    def initialize
    ...
    def each_incomplete_self
      @unseen_fields_for_each_self_reference.each { |info, unseen_fields|
        yield info unless unseen_fields.empty?
      }
    end
  end
  ...
  class Pass < Compiler::Pass::Analyze(Nil, Nil, Analysis)
    ...
    def analyze_func(ctx, f, f_link, t_analysis) : Analysis
      ...
      PreCompleteness.check_constructor(ctx, f, f_link, Analysis.new)
    end
  end
end
