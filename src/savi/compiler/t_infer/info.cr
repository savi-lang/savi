module Savi::Compiler::TInfer
  ...
  abstract class Info
    ...
    def as_downstream_constraint_meta_type(ctx : Context, type_check : TTypeCheck::ForReifiedFunc) : MetaType?
    ...
    def as_multiple_downstream_constraints(ctx : Context, type_check : TTypeCheck::ForReifiedFunc) : Array({Source::Pos, MetaType})?
      nil
    end
  end
  ...
  abstract class DynamicInfo < Info
    ...
    def total_downstream_constraint(ctx : Context, type_check : TTypeCheck::ForReifiedFunc)
    ...
    def list_downstream_constraints(ctx : Context, type_check : TTypeCheck::ForReifiedFunc)
    ...
    def describe_downstream_constraints(ctx : Context, type_check : TTypeCheck::ForReifiedFunc)
  ...
  abstract class NamedInfo < DynamicInfo
    ...
    def explicit_is_type_expr_cap? : Bool
      ...
      TInfer.is_type_expr_cap?(explicit.node)
    end
  ...
  class TowardCallParam < DynamicInfo
    ...
    def as_multiple_downstream_constraints(ctx : Context, type_check : TTypeCheck::ForReifiedFunc) : Array({Source::Pos, MetaType})?
      ...
      ctx.t_infer[rf.link].each_called_func_within(ctx, rf, for_info: @call) { |other_info, other_rf|
        ...
        pre_infer = ctx.pre_t_infer[other_rf.link]
