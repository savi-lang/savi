describe Mare::Compiler::Sugar do
  it "inserts a None return value where it was left out" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "return_none")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:string, "this isn't the return value"],
      [:ident, "None"],
    ]

    func = ctx.namespace.find_func!(source, "Example", "behave")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:string, "this isn't the return value"],
      [:ident, "None"],
    ]
  ...
  it "transforms a property assignment into a method call" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "prop_assign")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:relate,
        [:ident, "x"],
        [:op, "."],
  ...
  it "transforms an operator into a method call" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "plus")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:relate,
        [:ident, "x"],
        [:op, "."],
  ...
  it "transforms an operator into a method call (in a loop condition)" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "countdown")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:group, "(", [:loop,
        [:group, "(", [:relate,
          [:ident, "x"],
  ...
  it "transforms a square brace qualification into a method call" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "square")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:relate,
        [:ident, "x"],
        [:op, "."],
  ...
  it "transforms a chained qualifications into chained method calls" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "chained")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:relate,
        [:relate,
          [:ident, "x"],
  ...
  it "transforms an @-prefixed identifier into a method call of @" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "selfish")
    func.body.not_nil!.to_a.should eq [:group, ":",
  ...
  it "adds a '@' statement to the end of a constructor body" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "new")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:relate, [:ident, "x"], [:op, "="], [:integer, 1_u64]],
      [:ident, "@"],
    ]
  ...
  it "transforms non-identifier parameters into assignment expressions" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "param_assigns")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:relate,
        [:ident, "@"],
        [:op, "."],
  ...
  it "transforms short-circuiting logical operators into choices" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "logical")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:choice,
        [
          [:choice,
  ...
  it "transforms an `as!` call into a subtype check in a choice" do
    ...
    func = ctx.namespace.find_func!(source, "Example", "type_cast")
    func.body.not_nil!.to_a.should eq [:group, ":",
      [:relate,
        [:group, "(",
          [:relate,
