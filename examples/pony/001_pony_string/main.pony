///
// This source code file is taken from Pony's builtin package and adapted
// here in various ways, used as a test sample of Pony's syntax and semantics
// to prove that we can interpret Pony syntax.
// There is still more to work out with the syntax and semantics,
// and even more to work out with the standard library.

// use @memcmp[I32](dst: CPointer[U8] box, src: CPointer[U8] box, len: USize)
// use @memset[CPointer[None]](dst: CPointer[None], set: U32, len: USize)
// use @memmove[CPointer[None]](dst: CPointer[None], src: CPointer[None], len: USize)
// use @strtof[F32](nptr: CPointer[U8] box, endptr: CPointer[CPointer[U8] box] ref)
// use @strtod[F64](nptr: CPointer[U8] box, endptr: CPointer[CPointer[U8] box] ref)
// use @pony_os_clear_errno[None]()
// use @pony_os_errno[I32]()

actor Main
  new create(env: Env) =>
    PonyString.create(10)
    env.out.print("Hello, World!")

class val PonyString //is (Seq[U8] & Comparable[String box] & Stringable)
  """
...
```
  ...
  new ref create(len: USize = 0) =>
    ...
    """
    this._size = 0
    this._alloc = len.min(len.max_value() - 1) + 1
    this._ptr = CPointer[U8]._alloc(this._alloc)
    this._set(0, 0)
  ...
  new val from_array(data: Array[U8] val) =>
    ...
    this._alloc = data.space()
    this._ptr = data.cpointer()._unsafe()
  ...
  new iso from_iso_array(data: Array[U8] iso) =>
    ...
    this._size = data.size()
    this._alloc = data.space()
    this._ptr = (consume data).cpointer()._unsafe()
    if this._alloc > this._size then
      this._set(this._size, 0)
    end

  new from_cpointer(str: CPointer[U8], len: USize, alloc: USize = 0) =>
    """
    ...
    if str.is_null() then
      ...
      this._size = len
      this._alloc = alloc.max(this._size.min(len.max_value() - 1))
      this._ptr = str._unsafe()
    end

  new from_cstring(str: CPointer[U8]) =>
    """
    ...
    if str.is_null() then
      ...
      while str._unsafe()._apply(i) != 0 do
        i = i + 1
      end
  ...
  new copy_cpointer(str: CPointer[U8] box, len: USize) =>
    """
    ...
    if str.is_null() then
      ...
      this._size = len
      this._alloc = this._size + 1
      this._ptr = CPointer[U8]._alloc(this._alloc)
      str._copy_to(this._ptr, this._alloc)
    end

  new copy_cstring(str: CPointer[U8] box) =>
    """
  ...
  //   this._size = encoded._1
  //   this._alloc = this._size + 1
  //   this._ptr = CPointer[U8]._alloc(this._alloc)
  //   this._set(0, encoded._2)
  //   if encoded._1 > 1 then
  //     this._set(1, encoded._3)
  //     if encoded._1 > 2 then
  //       this._set(2, encoded._4)
  //       if encoded._1 > 3 then
  //         this._set(3, encoded._5)
  //       end
  //     end
  //   end
  //   this._set(this._size, 0)
  ...
  //   let encoded = _UTF32Encoder.encode(value)
  //   let i = this._size
  //   this._size = this._size + encoded._1
  //   reserve(this._size)
  //   this._set(i, encoded._2)
  //   if encoded._1 > 1 then
  //     this._set(i + 1, encoded._3)
  //     if encoded._1 > 2 then
  //       this._set(i + 2, encoded._4)
  //       if encoded._1 > 3 then
  //         this._set(i + 3, encoded._5)
  //       end
  //     end
  //   end
  //   this._set(this._size, 0)

  fun box _copy_to(ptr: CPointer[U8] ref, copy_len: USize,
    from_offset: USize = 0, to_offset: USize = 0) =>
    """
    Copy copy_len characters from this to that at specified offsets.
    """
    this._ptr._offset(from_offset)._copy_to(ptr._offset(to_offset), copy_len)

  fun cpointer(offset: USize = 0): CPointer[U8] tag =>
    """
    Returns a C compatible pointer to the underlying string allocation.
    """
    this._ptr._offset(offset)

  // fun cstring(): CPointer[U8] tag =>
  //   """
  ...
  //   let ptr = CPointer[U8]._alloc(this._size + 1)
  //   this._ptr._copy_to(ptr._unsafe(), this._size)
  //   ptr._update(this._size, 0)
  //   ptr
  ...
  //   """
  //   recover
  //     Array[U8].from_cpointer(this._ptr._unsafe(), this._size, this._alloc)
  //   end
...
//     n
  ...
  fun space(): USize =>
    ...
    """
    if this.is_null_terminated() then this._alloc - 1 else this._alloc end
  ...
  fun ref reserve(len: USize) =>
    ...
    null terminator.
    """
    if this._alloc <= len then
      let max = len.max_value() - 1
      let min_alloc = len.min(max) + 1
      if min_alloc <= (max / 2) then
        this._alloc = min_alloc.next_pow2()
      else
        this._alloc = min_alloc.min(max)
      end
      this._ptr = this._ptr._realloc(this._alloc)
    end
  ...
  fun ref compact() =>
    ...
    """
    if (this._size + 1) <= 512 then
      if (this._size + 1).next_pow2() != this._alloc.next_pow2() then
        this._alloc = (this._size + 1).next_pow2()
        let old_ptr = _ptr = CPointer[U8]._alloc(this._alloc)
        old_ptr._copy_to(_ptr, this._size)
        this._set(this._size, 0)
      end
    elseif (this._size + 1) < this._alloc then
      this._alloc = (this._size + 1)
      let old_ptr = _ptr = CPointer[U8]._alloc(this._alloc)
      old_ptr._copy_to(_ptr, this._size)
      this._set(this._size, 0)
  ...
  fun ref recalc() =>
    ...
    while (s < this._alloc) and (this._ptr._apply(s) > 0) do
      s = s + 1
    end

    if s != this._alloc then
      this._size = s
    end
  ...
  fun ref truncate(len: USize) =>
    ...
    """
    if len >= this._alloc then
      this._size = len.min(this._alloc)
      this.reserve(this._alloc + 1)
    else
      this._size = len.min(this._alloc - 1)
    end
  ...
  fun ref trim_in_place(from: USize = 0, to: USize = -1) =>
    ...
    this._alloc = if last == this._size then this._alloc - offset else size' end
    ...
    if this._alloc == 0 then
      this._ptr = CPointer[U8]._null()
    else
...
//     end
  ...
  fun is_null_terminated(): Bool =>
    ...
    """
    (this._alloc > 0) and (this._alloc != this._size) and (this._ptr._apply(this._size) == 0)
...
//     end
  ...
  fun at_offset(offset: ISize): U8 ? =>
    ...
    of bounds.
    """
    this.apply(this.offset_to_index(offset))? // TODO: apply sugar instead of this.apply?
...
//     _size <= that._size
  ...
  fun offset_to_index(i: ISize): USize =>
    if i < 0 then i.usize() + this._size else i.usize() end
...
//     if index < _size then
//       @pony_os_clear_errno()
//       var endp: CPointer[U8] box = CPointer[U8]
